...
import { computeQeegAnalysis, computeWPLI, computeCrossCorrelation } from "./qeeg.js";
...
// ICA constants + trainICA/applyTrainedICA moved to ./ica.js (imported above).
...
// Deterministic chronological ordering of a subject's recordings (Subject Timeline +
// before→after Compare). HIPAA Safe Harbor generalizes dates to the year (YYYY-01-01), so
// same-year visits tie on `date` alone and would otherwise fall back to arbitrary insertion
// order. Break the tie by the record's entry timestamp (uploadedAt — the best chronological
// proxy once the real day/month is gone), then by clinical study-type progression
// (Baseline → Post-Injury → Follow-Up → Routine → Long-Term), then filename — so the order
// is stable and clinically sensible instead of arbitrary. NOTE: within a year this is a
// best-effort proxy, not the true acquisition order (which the de-identified date no longer
// carries); a stored relative-day offset would restore that but needs the real dates at
// import time.
const STUDY_TYPE_ORDER = { BL: 0, PI: 1, FU: 2, RT: 3, LT: 4 };
function compareRecordChronology(a, b) {
  const d = (a.date || "").localeCompare(b.date || "");
  if (d !== 0) return d;
  const u = (a.uploadedAt || "").localeCompare(b.uploadedAt || "");
  if (u !== 0) return u;
  const ra = STUDY_TYPE_ORDER[a.studyType] ?? 99, rb = STUDY_TYPE_ORDER[b.studyType] ?? 99;
...
// ── FastICA artifact removal → ./ica.js (trainICA / applyTrainedICA, imported above) ──
// Extracted so the decomposition is unit-testable (it can't be exercised live — the seed
// recordings carry no EOG/EKG reference channels) and now uses a seeded PRNG for the
// weight init, making the output reproducible run-to-run. See test/ica.test.js.
...
// computeCrossCorrelation, computeWPLI, detectArtifacts, removeLineNoiseSpectral and
// computeAperiodicSlope now live in ./qeeg.js so the DSP web worker can run the whole qEEG
// pipeline off the main thread. WPLI + cross-correlation are re-imported above for the
// EOG-metrics panel below.
...
function QuantAnalysisPanel({ waveformData: _liveWaveform, channels, sampleRate, epochSec, epochStart, onClose, panelPos, setPanelPos }) {
  ...
  // power, WPLI eye-sync and IRASA aperiodic slope) now runs on the DSP web worker via
  // computeQeegAnalysis — zero main-thread cost even when the epoch settles. useDspJob keeps
  // the previous result on screen while the next epoch computes (last-request-wins), so
  // scrolling never flashes the panel to empty and never blocks waveform review.
  const { result: analysis } = useDspJob(
    "qeeg",
    () => (waveformData && waveformData.length) ? { waveformData, channels, sampleRate } : null,
...
function ComparePanel({ records, edfFileStore, onClose, panelPos, setPanelPos, ensureEdfLoaded }) {
  ...
  const allFiles = useMemo(
    () => [...(records || [])].sort(compareRecordChronology),
    [records]
  );
